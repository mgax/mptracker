...
class CabinetMembership(db.Model):
    ...
    mp_group_id = db.Column(UUID, db.ForeignKey('mp_group.id'), nullable=False)
    mp_group = db.relationship('MpGroup',
        backref=db.backref('cabinet_memberships', lazy='dynamic',
                           cascade='all'))
...
class Mandate(db.Model):
    ...
    election_votes = db.Column(db.Integer)
    election_votes_percent = db.Column(db.Numeric)
...
class Proposal(db.Model):
    ...
    url = db.Column(db.Text)
    pdf_url = db.Column(db.Text)
    cdeppk_cdep = db.Column(db.Integer, unique=True)
    cdeppk_senate = db.Column(db.Integer, unique=True)
    number_cdep = db.Column(db.Text, unique=True)
...
class VotingSession(db.Model):
    ...
    cabinet_choice = db.Column(db.Text)
